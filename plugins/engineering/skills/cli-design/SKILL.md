...
- **Flags name choices, the environment names context.** When a second subcommand needs the connection details a first one already takes (endpoint, region, profile), the shared part moves to the ecosystem's env contract — `AWS_ENDPOINT_URL_S3`/`AWS_REGION` for S3-compatible stores, `PGHOST`/`PGPORT` for Postgres — and each subcommand keeps one flag naming its own target, URL-shaped where the target has a URL form (`--backup-s3 s3://bucket/prefix` — one flag per purpose where a `--backup-s3-*` family would grow five). Credentials are shared context too, but they follow the next rule: the ecosystem's file-based path (`~/.aws/credentials` selected by `AWS_PROFILE`, `.pgpass` via `PGPASSFILE`), never a flag or an environment value. The general rule and its timing live in the `code-craftsmanship` skill when it is among your available skills (context once, choices per use).
