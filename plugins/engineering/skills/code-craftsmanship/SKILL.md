...
- **Context once, choices per use.** Every parameter on a surface — CLI flag, env var, config key, manifest field, function argument, API field — is either *context* (identical for every use on a host or in a run: how a service is reached, credentials, region) or a *choice* (what this particular use targets or does). Configure context in one place, reusing an ecosystem contract where one exists (`AWS_*` and its shared credentials file, `PG*` and `.pgpass`, `XDG_*`) before inventing your own — credentials in the contract's file-based part, never in flags or environment values — and hand each use only its choices. The trigger is the second occurrence — earlier than the rule of three above, because a surface is a contract: a parameter group cloned under a new prefix is knowledge every consumer has already learned, so it is factored before it ships — at two copies, where code waits for three.
